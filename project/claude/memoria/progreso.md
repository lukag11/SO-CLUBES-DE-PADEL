...
- **Pendiente:** deploy S2/S3 + prueba de pago end-to-end con cuenta conectada (webhook /:clubId acredita con el token del club). Luego S4 (UI conectar/desconectar + GET /api/mp/estado), S5 (QA 2 clubes). Follow-ups seguridad: firma webhook obligatoria en prod + lock en refresh (concurrencia).
