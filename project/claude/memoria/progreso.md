...
**Última actualización:** 2026-07-10 (tarde) — **SESIÓN DE CORRECCIONES + AUDITORÍAS PRE-DEPLOY** — categorías anti-duplicado por proximidad, Stock colapsable + filtro por estado, fixes en WIarky (`crear_reserva`), y sobre todo se cazó y cerró una clase de bug crítico: **"guardado fantasma"** (config del club que decía "guardado" sin persistir). Se auditó WIarky (4 profesionales) y el guardado fantasma (3 agentes). Ver bloque de hoy abajo.
...
- **PENDIENTE WIarky (roadmap):** (1) adjuntar FOTO en el chat para cargar gasto con IA mandándole la factura (reusa OCR `ocrGasto.js`) — pedido de Luca; (2) voz en iPhone vía Whisper server cuando aparezca; (3) insight con botón versión backend más fina; (4) prompt caching; (5) enchufar heatmap / TF en riesgo. Ver [[project_wiarky_auditoria_2026_07]].
