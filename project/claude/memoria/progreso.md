...
**Última actualización:** 2026-07-08 — **RECONCILIACIÓN Costo↔Gasto por DETECCIÓN automática** (#4 tanda financiera): fin de la doble contabilidad, el gasto pagado marca el costo pagado solo. Antes: resultado del mes + comisión MP + costo por turno. Ver abajo.
...
- **PRÓXIMO BLOQUE GRANDE (NO hecho) — AI-OCR de gastos con WIarky:** el dueño sube foto/PDF de una factura (luz, agua, internet, ticket) → Claude visión extrae proveedor/concepto/monto/fecha/N° factura/categoría → pre-llena el gasto para confirmar → guarda con imagen adjunta y `fuente: 'ocr'` (el campo YA existe en el modelo `Gasto`, anticipando esto). Es la dirección "WIarky caballo de batalla / automatización + simplicidad". **Decisiones abiertas:** entrada (botón en Gastos vs WIarky), modelo (Haiku vs Sonnet, para no errarle al monto), PDF (conversión). Ver [[project_wiarky_caballo_batalla]].
