...
6. **WIarky — TOOL USE (paso "acciones", 2026-06-22).** `responderChatAgente` corre el loop de function calling de Claude con `WIARK_TOOLS`. Skills actuales (8): lectura → `consultar_disponibilidad`, `consultar_deudores` (lista PII-safe vía artefacto), `consultar_ingresos`; generación → `armar_posteo_disponibilidad`, `armar_convocatoria` (artefactos copiables); escritura con confirmación → `cargar_gasto`, `crear_reserva`, `crear_jugador`. Artefactos al front: `CopyArtefacto` (texto copiable), `ListaArtefacto` (lista, ej. deudores — nombres NUNCA pasan por la IA), `ConfirmAccion` (card de confirmación de writes). **Regla de oro:** el chat NUNCA escribe; los writes reusan endpoints blindados (`/me/insight/accion` para gasto, `/reservas/admin` para reserva, `/jugadores` para alta) solo tras confirmación. Extensible: nueva acción = una herramienta + (si escribe) ruteo en `ConfirmAccion`. **Falta:** más writes (cobrar, bloquear turno) y voz.
