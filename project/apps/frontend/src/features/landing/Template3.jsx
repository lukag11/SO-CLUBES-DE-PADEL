...
const Template3 = ({ club, onCta, onTorneos }) => {
  const { colorPrimario, nombre, logo, canchas, horarios,
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      ...
      <section className="min-h-screen flex items-center px-6 pt-16">
        <div className="max-w-6xl mx-auto w-full grid md:grid-cols-2 gap-20 items-center py-28">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2">
              ...
              <button onClick={onTorneos} className="text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors px-2">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div
                key={title}
                onClick={() => title === 'Torneos' ? onTorneos() : onCta()}
                style={{ '--clr': colorPrimario }}
                className="group cursor-pointer flex flex-col gap-4 p-7 rounded-2xl bg-white border border-slate-200 transition-all duration-200 hover:-translate-y-1 hover:border-[var(--clr)] hover:shadow-lg hover:shadow-slate-100"
              >
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition-transform duration-200 group-hover:scale-110" style={{ backgroundColor: `${colorPrimario}15` }}>
                  <Icon size={22} style={{ color: colorPrimario }} />
                </div>
                <div>
                  <h3 className="text-slate-800 font-bold text-base mb-1.5">{title}</h3>
      ...
      <TorneosSection colorPrimario={colorPrimario} dark={false} onCta={onCta} soloEnCurso />

      {/* TURNOS DISPONIBLES */}
      {(seccionesVisibles?.reservas ?? true) && (
        <section id="reservas" className="py-32 px-6 bg-white border-y border-slate-100">
          <div className="max-w-5xl mx-auto">
      ...
      <PartidosAbiertosSection colorPrimario={colorPrimario} dark={false} />
      ...
      {galeria?.length > 0 && (seccionesVisibles?.galeria ?? true) && (
        <section id="galeria" className="py-32 px-6 bg-white border-y border-slate-100">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-black text-slate-900 mb-4">Instalaciones</h2>
            <p className="text-slate-400 mb-14">Conocé nuestras canchas y espacios.</p>
      ...
      {servicios?.some((s) => s.activo) && (seccionesVisibles?.servicios ?? true) && (
        <section id="servicios" className="py-32 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-slate-400 mb-14">Todo lo que encontrás en el club.</p>
      ...
      {staff?.length > 0 && (seccionesVisibles?.staff ?? true) && (
        <section id="equipo" className="py-32 px-6 bg-white border-y border-slate-100">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-black text-slate-900 mb-4">Nuestro equipo</h2>
            <p className="text-slate-400 mb-14">Las personas detrás del club.</p>
      ...
      {faq?.length > 0 && (seccionesVisibles?.faq ?? true) && (
        <section id="faq" className="py-32 px-6">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-slate-400 mb-14">Todo lo que necesitás saber antes de reservar.</p>
      ...
      <section className="py-32 px-6 bg-slate-900">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-white/50 mb-14">Creá tu cuenta gratis y empezá a reservar canchas hoy mismo.</p>
      ...
      <footer className="bg-slate-900 border-t border-white/5 py-10 px-6 text-center">
        <div className="flex items-center justify-center gap-2 mb-3">
          <div className="w-6 h-6 rounded-md flex items-center justify-center overflow-hidden" style={{ backgroundColor: logo ? 'transparent' : colorPrimario }}>
            {logo ? <img src={logo} alt={nombre || 'Club'} className="w-full h-full object-contain" /> : <Zap size={12} className="text-white" />}
          </div>
