...
**RN-64** La **comisión del medio de pago** (`club.config.comisionPorMetodo = { mercadopago: 3.5, ... }`, default vacío = 0%) es la "mordida" que el medio de cobro le hace al **CLUB**, no un recargo al cliente. Se descuenta **solo en el ANÁLISIS de rentabilidad** (Dirección: break-even, precio realizado, margen por sector — helpers `comisionDeMetodo`/`netoRealizado` en `lib/finanzas.js`). La **deuda es siempre el BRUTO** y el **arqueo cuenta el efectivo físico** — ninguno se toca. Opt-in: sin comisión cargada, los números son idénticos a hoy.  
**RN-65** El puente **Costo↔Gasto** es por **DETECCIÓN AUTOMÁTICA**, sin doble carga. Un **Costo** es estructura (alimenta break-even/resultado del mes); un **Gasto** es la factura real pagada (Pagos → Gastos). El sistema marca un costo como **`pagadoEsteMes`** (calculado en `GET /costos`) cuando hay un gasto **pagado este mes** que lo cubre, por dos vías: **(1)** vínculo explícito `Gasto.costoId`, o **(2)** **mismo concepto normalizado** (trim + lowercase; ej. el gasto "Alquiler" cubre el costo "Alquiler"). **No hay botón "registrar pago" ni doble carga**: el dueño carga el gasto como siempre y el sistema detecta solo (se descartó una versión con endpoint `POST /costos/:id/pagar` por generar doble vía y duplicados). La detección funciona **mejor con costos itemizados** (Alquiler, Luz, Sueldos por separado) que matcheen el nombre del gasto que uno más los agrupa. UI: en `CostosPanel` cada costo fijo detectado muestra "✓ Pagado"; en Dirección la tarjeta "Tus costos de este mes" (estructura $X · pagaste $Y · falta $Z + barra) se calcula en el front desde la lista con `pagadoEsteMes`.  
...
**RN-66** El **OCR de gastos con IA** (dueño sube foto de una factura/ticket/recibo → la IA carga el gasto) tiene tres reglas duras: **(1) NUNCA guarda solo** — el endpoint `POST /api/gastos/extraer` (`lib/ocrGasto.js`, Claude visión Haiku 4.5) **solo LEE** y devuelve los datos (proveedor/concepto/monto/fecha de emisión/vencimiento/N° factura/categoría) para que el dueño **CONFIRME**; la creación sigue por `POST /api/gastos`. La plata se revisa siempre. **(2) Formato argentino de números** en la extracción: el **punto** separa MILES y la **coma** los DECIMALES (centavos) — se le enseña con ejemplos ("$22.560,30" → `22560`) porque sin eso subía el monto ×100 ($2.256.030). El monto es entero en pesos (se descartan centavos). Se distingue explícitamente **fecha de emisión** de **VENCIMIENTO**. **(3) El Gasto guarda `vencimiento String?`** (fecha en que hay que pagar) y el sistema **avisa antes de que venza**: en la lista de gastos impagos, chip semáforo (rojo = vencido/vence hoy, amarillo = vence en ≤5 días, gris = más lejos); y **WIarky** avisa proactivamente en criollo (línea "Facturas por pagar" en el contexto del chat + bullet en el system prompt: "Ojo, la luz vence el 05/08"). Si la IA leyó un vencimiento futuro, el gasto arranca en estado "A pagar" (impago). Una foto de celular pesa varios MB → el parser de `/api/gastos/extraer` usa límite de 15mb, montado antes del global (el límite global de 8mb la rechazaba).  
**RN-67** Refinamiento del OCR de gastos (afinado con facturas reales de club): **(1) Categorías por rubro de club** — la IA clasifica en `servicios / alquiler / sueldos / impuestos / bebidas / kiosco / deportivo / insumos / mantenimiento / otros` (antes eran solo servicios: energia/agua/internet/…), con guía por rubro en el prompt (factura de distribuidor de bebidas → `bebidas`; de artículos de pádel → `deportivo`). El `CATEGORIAS` del server, el `CAT_IA` y `CATEGORIAS_SUGERIDAS` del front están alineados. **(2) El OCR NO guarda la foto** — el flujo IA lee la factura como un **scanner** y DESCARTA la imagen (no se sube a Storage, no se persiste). Motivo: simplicidad + menos costo de storage + menos puntos de falla. Se quitó el botón "Adjuntar foto" del modal. El campo `Gasto.imagenUrl` queda **LATENTE** en la DB (apagado, costo cero) por si un plan premium futuro quiere "archivo de facturas" — se prende en 10 min. Regla: el feature se construye cuando alguien lo pide/paga, no "por si acaso". **(3) CONTADO vs Vto CAE** — la IA distingue el `vencimiento` (fecha de PAGO, solo facturas a crédito) del **"Vto CAE"/"CAE"** (dato fiscal de AFIP, se IGNORA — bug real que revelaban las facturas). Nuevo campo `contado` (boolean): si la factura dice CONTADO → `contado:true`, sin vencimiento, y en el front arranca como **PAGADA**. **(4) Campos opcionales `Gasto.cuitProveedor` + `Gasto.tipoComprobante`** — la IA los extrae (CUIT del emisor, tipo A/B/C/Ticket/Remito/Recibo), el modal los muestra auto-llenados (datalist), pero son **opcionales**: no se agrega NADA obligatorio nuevo (la carga sigue rápida: concepto + monto). **(5) Detector de facturas duplicadas (AVISO, no bloqueo)** — al CREAR un gasto (no al editar) se compara contra los ya cargados: match **fuerte** = proveedor + N° de factura; match **débil** (si no hay N°) = proveedor + monto + fecha. Si coincide → banner ámbar "¿Ya la cargaste?" no bloqueante (el dueño decide). Puro frontend. **(6) Filtro por proveedor + total** — dropdown de proveedores (auto-cargados por la IA) que combina con los filtros de estado/categoría; al filtrar un proveedor aparece el banner "Le pagaste a X ($total, N facturas)" y el reporte/CSV salen filtrados. Gotcha: nombres inconsistentes del mismo proveedor pueden fragmentar (se edita a mano).  
**RN-68** El **OCR de facturas también carga el STOCK** (Etapa 1 del círculo OCR → Stock → Margen): además de los datos de la factura, la IA lee los **ÍTEMS** (renglones de producto) y devuelve `items: [{ descripcion, nombreLimpio, bultos, unidadesPorBulto, importe }]`. Reglas: **(1)** la IA **propone, el dueño confirma** — nadie carga stock a ciegas; en el modal de gasto, cuando la factura trae ítems y el rubro es `bebidas`/`kiosco`/`deportivo`, aparece la tabla "¿Cargar estos productos al stock?" (toggle, **default ON en esos rubros**) donde el dueño mapea cada ítem a un producto del club (o crea uno nuevo) y ajusta unidades/costo. **(2)** La **unidad real la calcula el CÓDIGO, no la IA** (Haiku calcula mal): `unidades = bultos × unidadesPorBulto` — la IA lee las dos por separado (columna BULTOS + el "4X6"/"X6" de la descripción; "4X6" = 4 packs de 6 = 24 unidades) pero NUNCA multiplica. El costo unitario se deriva `importe / unidades` y se recalcula al editar las unidades. **(3)** `nombreLimpio` es el nombre corto/de catálogo que sugiere la IA ("Pepsi") en vez del código del proveedor ("PEPSI 70KCAL 4X6 354CC"); se matchea contra los productos existentes por nombre para no duplicar (se muestra el nombre crudo de factura como referencia). **(4)** Al confirmar, el gasto se crea con `lineasStock` en `POST /api/gastos` (backend ya existente): matchea el producto por nombre o lo crea + `ingresarStock` suma stock y actualiza el costo → alimenta el COGS/margen. **(5)** Antes de elegir la foto se muestra un modal de tips ("sacá la mejor foto" + aviso "revisá antes de guardar, vos confirmás"), con "no mostrar más" (localStorage `gastos_ia_tips_ok`). **PENDIENTE (unificación decidida):** el "Ingresar compra" del tab Stock y el flujo de Gastos hacen ambos `POST /gastos` con `lineasStock` (duplicado, riesgo de doble carga) → se unifica por la opción A (sacar el form de Stock, su botón lleva al flujo de Gastos).
