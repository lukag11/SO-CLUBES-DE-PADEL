...
router.post('/', requireRole('jugador'), async (req, res) => {
  ...
  try {
    ...
    // Notificar (in-app) a los jugadores de esa categoría SOLO si es público.
    // Privado = solo por link (para tu grupo), no se avisa a la categoría.
    let notificados = 0
    if (cat && vis === 'publica') {
    ...
    // El link al lobby existe SIEMPRE (pública y privada): es por donde se entra desde el celular.
