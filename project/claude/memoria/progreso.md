...
**Última actualización:** 2026-07-23 — **RED UNIFICADA — E0.1 (cimiento).** Arrancó la migración a red unificada ([[project_red_unificada_arquitectura]]): un dominio, club por path (`padelwiark.app/club/tancacha`), cuenta jugador global. **E0.1 hecho:** helper `frontend/src/lib/clubContext.js` (`getClubSlug()` lee el slug de la URL `/club/:slug`, si no del env `VITE_CLUB_SLUG`) + los 9 usos del slug migrados al helper (clubStore, LandingPage, EventosPage, PartidosPublicosPage, PlayerEventosPage, LandingSections, TorneoPublicoPage, TorneosPublicosPage). NO-OP hoy (sin `/club/` en la URL → devuelve el env, comportamiento idéntico, verificado). `VITE_CLUB_ID` (login) intacto → se rehace en E2. **E0.2 hecho:** `getClubSlug()` ahora es "sticky" (recuerda el slug de la URL en sessionStorage para que la nav a rutas planas no pierda el club) + ruta `/club/:slug` → LandingPage en el router. **Verificado en vivo con 2 clubes** (club-demo verde vs rio-tercero rojo — segundo club de prueba creado en DB): la MISMA app muestra clubes distintos según `/club/:slug`. Objetivo central de E0 LOGRADO en la landing. **Pendiente (pulido/E0.3):** que la URL arrastre `/club/xxx` en toda navegación (hoy sticky lo cubre por detrás; parte se resuelve en E2 con el club en el token). **E1 HECHA:** modelo `Cuenta` global (dni @unique, email, password, tokenVersion + datos de persona) + `Jugador.cuentaId` (nullable). Aplicado con `db push` (OJO: el .env de dev apunta al **Supabase remoto del demo**, no a una base local — salió bien porque fue aditivo y es data de demo; pendiente urgente: separar local/staging, ver [[project_manual_programador_pendiente]]). Backfill: 29 cuentas creadas (0 DNIs repetidos) + 29 jugadores linkeados, 0 sin cuenta. **Doble escritura** (Jugador conserva sus campos) → app idéntica, verificada (health OK, Cuenta→membresías OK). La Cuenta la USA recién la E2. **E2.1 HECHA:** login jugador GLOBAL (`/auth/jugador/login` verifica DNI+password vs `Cuenta`, resuelve club desde membresías; `needsClubChoice` si tiene varias; token lleva `clubId` activo + `accountId` → las 203 rutas intactas). Backward-compatible (front que manda clubId sigue igual). Verificado (login sin clubId OK, errores cortan). **E2.2 HECHA:** `GET /auth/jugador/me` (jugador + sus membresías, para el selector) + `POST /auth/jugador/switch-club` (valida membresía activa en el club destino → re-emite token; 403 si no es miembro). Verificado. **E2.3a HECHA:** `PlayerAuthPage` login manda solo DNI+password (sin `VITE_CLUB_ID`). **E2.3b HECHA:** selector de club (modal) cuando la cuenta tiene varios → elige → login con ese clubId → entra. Con **logo de cada club** (o inicial si no tiene). **E3 (parcial) HECHA:** endpoint `POST /auth/jugador/unirme` (jugador logueado crea membresía en otro club, copia datos de la Cuenta, no duplica). **PROBADO EN VIVO EL LOOP COMPLETO:** Luca (DNI 38110765) se unió a "Rio Tercero Padel" (2º club de prueba creado en DB) → login muestra selector con Club Demo + Rio Tercero → elige y entra. **NÚCLEO DE LA RED UNIFICADA (E0→E3) FUNCIONANDO.** **E3 FRONTEND HECHA:** `UnirmeBanner.jsx` en la landing — jugador logueado NO miembro del club que ve → banner "¿Jugás en X? Unirme" → `POST /unirme` → socio. Probado el círculo COMPLETO en vivo: login → landing de otro club → Unirme → logout/login → selector con los 2 clubes (con logos). + fix: `clubStore.loadFromBackend` ahora usa `Club.nombre` (columna) como fallback si `config.nombre` no está (antes el banner/nombre salía vacío para clubes sin config.nombre). **RED UNIFICADA E0→E3 COMPLETA de punta a punta.** Falta (pulido, deferible): ruteo slug-aware del área logueada (que la URL del dashboard lleve el club, hoy va por sesión — observación de Luca); PWA+push (E4). **E2.3c HECHA:** switcher in-app (`ClubSwitcher.jsx` en el sidebar del PlayerLayout) - si la cuenta tiene varios clubes, desplegable para cambiar sin desloguear (switch-club re-emite token + window.location recarga). **E2 COMPLETA de punta a punta.** Test data: Luca (DNI 38110765) socio de Club Demo + Rio Tercero. Migración: E0.1·E0.2·E1·E2.1·E2.2·E2.3a·E2.3b·E3(back) commiteadas.

**PWA INSTALABLE — E4.1 (2026-07-23).** La app ya se puede "instalar" en el celu/desktop (una sola app "PadelwIArk", sobre la base unificada). Hecho: **íconos** generados con sharp desde el logo real de Luca (`C:\Users\luk_1\Downloads\PadelwIArk.png` — hexágono neón paleta+cerebro; recortado el emblema sin el texto, crop {left:237,top:100,w:780,h:780}) → `public/icons/` (icon-192, icon-512, icon-maskable-512 sobre fondo #0b0b18, apple-touch-icon-180) + favicon-32/48 nuevos (reemplazan el rayo violeta viejo `favicon.svg`, que quedó sin referenciar). `public/manifest.webmanifest` (name PadelwIArk, display standalone, theme/bg #0b0b18). `public/sw.js` **service worker mínimo: NO cachea NADA de la API** (cero data vieja de plata; passthrough) — el push (E4.2) se agrega ahí después. `index.html`: título PadelOS→PadelwIArk + metas PWA (theme-color, manifest, apple-touch, apple-mobile metas) + favicon nuevo. SW registrado en `main.jsx`. **Probado: se instala en desktop (Chrome/Edge) con el logo, ventana propia.** OJO marca: el logo nuevo es **azul/violeta neón** (antes la marca era lima #afca0b) — Luca eligió el neón. Falta **E4.1b** (botón "Instalá la app" beforeinstallprompt Android + instructivo iOS "Compartir→Agregar a inicio") y **E4.2** (push: VAPID + web-push + PushSubscription + soft-ask). Ver [[project_push_celular_postdeploy]].

**ASISTENTE DE BIENVENIDA v1 + CHECKLIST "Completá tu club".** Onboarding: club nuevo (0 canchas) → wizard full-screen (canchas/horario/precio/logo) → dashboard usable. + tarjeta checklist en el Resumen que guía a completar el resto (logo/contacto/MP/descripción), se esconde al 100%. + Cerró planes+billing (test bloqueo licencia vencida + fix mensaje). + Se destrabó git (chkdsk reparó `.git/objects/ff` corrupto). Ver [[project_asistente_bienvenida]] y [[project_planes_billing_rediseno]].

**SENTRY refinamiento + contacto ventas + limpieza (2026-07-23).** (#1) **Contacto de ventas configurable** ([[project_contacto_ventas_platform]]): el modal "Mejorar plan" ya no tiene WhatsApp hardcodeado; se carga en /plataforma→Perfil (selector país+bandera real, área/número). (#3) **Limpieza data de prueba:** borrados 2 clubes test (Padel max/pro) + 7 convocatorias de junio; queda solo Club Demo. (#4) **Sentry blindado:** `lib/sentry.js` (init + puente `console.error→Sentry.captureException` para Error instances → cubre los ~60 catches que responden 500 sin propagar, sin tocarlos) + `captureException` explícito con contexto en el catch del webhook de MP (punto más crítico). Dormido sin SENTRY_DSN. (#2) **Matching caso 1 resuelto:** en `PlayerReservasPage`, la card "armá tu partido" estado C (jugador SIN reserva) tenía un texto pasivo "Elegí un horario" que no hacía nada → ahora es un botón real que scrollea (block:'center') a la grilla + le da un destello/glow lima (`destacarHorarios`, 1.6s) a la sección "Horarios disponibles". Cero cambios en la lógica de reservar. **BLOQUE DE CHICOS 100% CERRADO.**

**PLANES renombrados + v2 asistente (manual) (2026-07-22).** Nombres comerciales Cancha/Club/Cadena → **Básico / Pro / Premium** (ids internos igual). En `PwPrecios.jsx` + `planesInfo.js` + comentarios `planes.js`. Unifica con el Sidebar. · **v2 del asistente (paste-and-parse):** se construyó el backend (`lib/onboardingParse.js` + `POST /clubs/me/onboarding/parse`, Haiku verificado por código) pero al probar el campo de IA en el paso 0, Luca lo rechazó: IA de portero en la 1ª pantalla = mal onboarding (el dueño nuevo no sabe quién es WIarky). **Alta queda 100% MANUAL**; el parse queda inerte para reusar ADENTRO de la app. Ver [[feedback_wiarky_copiloto_no_piloto]].

**ASISTENTE DE BIENVENIDA v1 (2026-07-22).** Al crear un club (signup) ya no cae en un panel vacío: `AdminDashboardLayout` decide una vez al cargar `/clubs/me` (dueño + 0 canchas reales + sin flag onboarding) → `AsistenteBienvenida.jsx` (wizard 5 pasos: bienvenida→canchas[cantidad/tipo/techado]→horario+precio→logo opcional→resumen). Trampa resuelta: el `clubStore` rellena 4 canchas demo si la DB tiene 0 → se agregó `_canchasReales` (verdad del backend). Persiste con acción atómica `aplicarOnboarding` + `saveClub` (PATCH /me config con `onboardingCompletado:true` + PATCH /me/canchas). Tipo/techado parejo a todas (v1 dead-simple); mixto se edita después. **CHECKLIST "Completá tu club":** `GET /clubs/me/setup` (determinista) + `SetupChecklist.jsx` en el Resumen (solo dueño), barra de progreso + pendientes con botón directo a Club, se esconde al 100%. **v2 futura:** paste-and-parse de WIarky (pegar "3 canchas de cristal, abro 8 a 23, turno $12k" → lo arma la IA).
